...
class WorkflowTransition(models.Model):
    ...
    def trigger_transition(self, active_record_id=None, active_model_name=None):
        self.ensure_one()
        if active_model_name and active_record_id and active_model_name == 'inquiry.request':
            ...
            if next_transition:
                ...
                if allowed_users_next_transition:
                    ...
                    record_id.write({'concerned_user_ids':  [(6, 0, allowed_users_next_transition.ids)], 'close_date': fields.Datetime.now()})
                    record_id.message_post(body=_("Status changed from %s To %s", self.state_from.name,self.state_to.name), message_type='comment', author_id=self.env.user.partner_id.id)
                else:
            ...
            # If this is the last transition
                ...
                message_approve = _('This inquiry request was approved: %s', record_id.sequence)
                ...
                notif_id = self.env['notification.system'].sudo().create({
                        ...
                        'name': _('Inquiry Request APPROVED'),
